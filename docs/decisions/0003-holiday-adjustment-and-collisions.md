...
Completed and expired occurrences are historical records and are never rewritten by refreshed holiday data. Only pending occurrences can be regenerated after a schedule, settings, or holiday-cache change.
...
- Provider changes or refreshed data cannot alter completed household history.
